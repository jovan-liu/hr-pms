...
<body>
	...
	<div class="container-fluid">
		<div class="row">
			...
			<br>
			<button class="btn btn-info" style="width:150px;height:40px;margin-right: 300px;" id="addButton">新增分类</button>
			<br><br>
					...
					<table id="dataTable" class="table table-striped">
						...
						<tbody id="data-cont">
							<c:forEach var="categoryList"
								items="${categoryList}">
								<tr id="dashboard-${categoryList.id}" class="post">
									<td>${categoryList.id}</td>
									<td>${categoryList.name}</td>
									<td>${categoryList.desc}</td>
									<td>
										<div class="btn-group">
											<button
												onclick="edit('${categoryList.id}')"
												type="button" class="btn btn-info" data-toggle="dropdown"
												style="margin-right: 10px;">编辑</button>
											<button
												onclick="delete2('${categoryList.id}')"
	...
	<script type="text/javascript">
		...
		$("#addButton").click(function(){
	 		$('#myModal').load("<%=request.getContextPath()%>/category/category-add-show",function(){
				$('#myModal').modal('show');
			});
	 	});
		...
		function edit(id) {
			$('#myModal').load(
					"<%=request.getContextPath()%>/category/category-edit-show",
					{"id":id},
					function() {
						$('#myModal').modal('show');
					});
		...
		function delete2(id){
			...
			$.post("${appName}/category/delete",{"id":id},
			function(data){
